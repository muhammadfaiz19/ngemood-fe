...
export default function History() {
  ...
  // FIX ERROR: Wrap loadData dalam useCallback agar stabil
  const loadData = useCallback(async () => {
    try {
      const res = await fetchWithAuth('/moods/history');
  ...
  // FIX ERROR: Panggil async function di dalam useEffect dengan benar
  useEffect(() => {
    loadData();
  }, [loadData]);
  ...
  // Mapping Angka ke Emoji (Untuk Label Y-Axis)
  ...
  // Siapkan Data Chart (Ambil 7 Terakhir & Reverse biar urut tanggal)
  ...
  const chartData = {
    ...
    datasets: [{
      ...
      backgroundColor: (context: ScriptableContext<'line'>) => {
        ...
        gradient.addColorStop(0, 'rgba(99, 102, 241, 0.5)'); // Atas pekat
        gradient.addColorStop(1, 'rgba(99, 102, 241, 0)');   // Bawah transparan
        return gradient;
      },
  ...
  const chartOptions: any = {
    ...
    scales: {
      y: {
        ...
        ticks: {
          stepSize: 1,
          callback: (value: number) => scoreToLabel(value), // Ganti angka jadi Emoji
          font: { size: 12 }
        },
        grid: { color: '#f1f5f9' }
      },
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 h-full">
      ...
      <div className="bg-white p-6 shadow-sm flex items-center justify-between sticky top-0 z-20 border-b border-slate-100">
         ...
         {data.length > 0 && (
            <button onClick={deleteAll} className="text-red-500 text-xs font-bold bg-red-50 px-3 py-2 rounded-lg hover:bg-red-100 transition">
                Reset Data
            </button>
         )}
      </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="space-y-3 pb-10">
                ...
                {data.map((item) => (
                    <div key={item.id} className="bg-white p-4 rounded-2xl flex items-center gap-4 shadow-sm border border-slate-50 transition hover:shadow-md hover:border-indigo-100 group">
                        {/* Icon Mood */}
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-inner ${
                            ...
                            {item.mood === 'senang' ? '🥰' : item.mood === 'sedih' ? '😢' : item.mood === 'marah' ? '😡' : item.mood === 'stres' ? '🤯' : '😐'}
                        </div>

                        {/* Text Info */}
                        <div className="flex-1">
                            <div className="flex justify-between items-start">
                                <p className="font-bold capitalize text-slate-800 text-lg leading-tight">{item.mood}</p>
